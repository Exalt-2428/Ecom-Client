...
const ProductPage = () => {
    ...
    const products = [
      {
        id: 1,
        imageSrc: "https://comicsense.b-cdn.net/wp-content/uploads/2023/08/gear-5-jersey-tee-scaled.jpg",
        title: "Product 1",
        availableSizes: ["S", "M"],
        allSizes: ["S", "M", "L", "XL"],
        price: "$20",
        ...
        const updateRemainingTimes = () => {
          ...
          products.forEach((product) => {
            if (product.showCountdown) {
              updatedRemainingTimes[product.id] = calculateRemainingTime(product.offerEndsAt);
            }
          });
    ...
    <Header />
      <div className="p-16">
        ...
        {products.slice(0, visibleProducts).map((product) => (
            ...
            <div key={product.id} className="bg-gray-100 p-4 text-center relative">
              ...
              <img src={product.imageSrc} alt={product.title} className="mx-auto mb-2" />
              <a href="#" className="text-black hover:underline">{product.title}</a>
                ...
                <div className="flex items-center justify-center space-x-1 mt-2">
                  {[1, 2, 3, 4, 5].map((rating) => (
                    <Icon
                      key={rating}
                      icon="ri:star-fill"
                      className={rating <= product.starRating ? 'text-red-500' : 'text-gray-400'}
                    />
                  ))}
                </div>
              )}
